...
<p align="center">
  <h1 align="center">Medical Appointment Manager (Clinicativa)</h1>
</p>
...
| :--- | :---------------- | :----------- |
| 🖥️ | **Hybrid Frontend** | Seamless transition between a text-based Terminal CLI loop and a full-featured Java Swing GUI Desktop dashboard. |
...
```text
medical-appointment-manager-OOP/
├── .github/                 # Automated script actions pipeline
├── docker-dev/              # Infrastructure orchestrations
│   └── init-db/             # Seed folder containing the relational table schema (init.sql)
├── Modelagem De Dados/      # Structural entity diagrams and relational maps
└── sistema-clinica/         # Main Maven project code framework
    ├── src/
    │   ├── main/java/com/artur/clinica/
    │   │   ├── app/         # Main runtime bootstrapper and Console text-loop utilities
    │   │   ├── Controller/  # MVC Controller intercepting and decoupling UI from backend services
    │   │   ├── exception/   # Custom contextual business rule exceptions (ConflitoHorarioException)
    │   │   ├── model/       # Polymorphic hierarchies (Consulta, ConsultaClinica, Cirurgia, Medico)
    │   │   ├── services/    # Pure JDBC Persistence layers (ConsultaPostgresDAO) and connection drivers
    │   │   └── view/        # Java Swing desktop components (MainFrame, OperacoesTabelaPanel, Dialogs)
    │   └── test/java/com/artur/clinica/
    │       ├── ConflitoHorarioTest.java     # Conflict schedule matrix tests
    │       ├── ConsultaIntegracaoTest.java    # Multi-stage integration workflow tests
    │       └── ConsultaValidacaoTest.java     # String metrics, regex masks, and calendar validations
...
* *If this project helped you understand OOP patterns, refactoring, or relational mapping database integration, consider giving it a ⭐!*
